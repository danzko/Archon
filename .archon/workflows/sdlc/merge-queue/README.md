...
and `mode=preview|approve|auto`. Assessment freezes the live batch, then two fresh
command reviews bound to `@review-anthropic` and `@review-zai` inspect that exact
head. The native final script verifies their typed verdicts and engine-recorded
routing before it can call GitHub.
...
findings, conflicts and pending checks also hold the remaining work. The script
...
`--match-head-commit`; GitHub-native queues remain queued until read-back confirms
merging. No admin bypass is used.
